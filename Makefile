# This Makefile is for the Interhack extension to perl.
#
...
#     DIR => []
#     DISTNAME => q[Interhack]
#     NAME => q[Interhack]
#     NO_META => q[1]
#     PL_FILES => {  }
#     PREREQ_PM => { IO::Pty::Easy=>q[0], Term::TtyRec::Plus=>q[0], Net::OSCAR=>q[0], IO::Socket::INET=>q[0], Time::HiRes=>q[0], IO::Socket=>q[0], Term::VT102=>q[0], File::Temp=>q[0], Term::ReadKey=>q[0], LWP::Simple=>q[0] }
#     VERSION => q[1.0]
...
TO_INST_PM = ih-server.pl \
	interhack.pl \
	lib/Interhack/Sock.pm \
	lib/nhgrep

PM_TO_BLIB = lib/Interhack/Sock.pm \
	blib/lib/Interhack/Sock.pm \
	lib/nhgrep \
	blib/lib/nhgrep \
	interhack.pl \
	$(INST_LIB)/interhack.pl \
	ih-server.pl \
	$(INST_LIB)/ih-server.pl
...
ppd:
	...
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="IO-Pty-Easy" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="IO-Socket" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="IO-Socket-INET" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="LWP-Simple" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Net-OSCAR" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Term-ReadKey" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Term-TtyRec-Plus" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Term-VT102" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' -- \
	  lib/Interhack/Sock.pm blib/lib/Interhack/Sock.pm \
	  lib/nhgrep blib/lib/nhgrep \
	  interhack.pl $(INST_LIB)/interhack.pl \
	  ih-server.pl $(INST_LIB)/ih-server.pl 
